"""price an option by the Leisen-Preimer tree"""
...
class BinomialLROption(BinomialTreeOption):
    def _setup_parameters_(self):
        ...
        d1=(math.log(self.S0/self.K)+((self.r-self.div)+(self.sigma**2)/2.)*self.T)\
            /(self.sigma*math.sqrt(self.T))
        d2=(math.log(self.S0/self.K)+((self.r-self.div)-(self.sigma**2)/2.)*self.T)\
            /(self.sigma*math.sqrt(self.T))
        pp_2_inversion=lambda z,n:0.5+math.copysign(1,z)*math.sqrt(0.25-
                        0.25*math.exp(-((z/(n+1./3.+0.1/(n+1)))**2.)*(n+1./6.)))
