...
class FDExplicitEu(FiniteDifferences):
    ...
    def _setup_boundary_conditions_(self):
        if self.is_call:
            ...
            self.grid[0,:-1]=self.K*np.exp(-self.r*self.dt*
                                                       (self.N-self.j_values))
    ...
    def _traverse_grid_(self):
        for j in reversed(self.j_values):
            for i in range(self.M)[1:]:
                self.grid[i,j]=self.a[i]*self.grid[i-1,j+1]+self.b[i]*self.grid[i,j+1]+\
                               self.c[i]*self.grid[i+1,j+1]

#-----------------------------------------
if __name__=="__main__":
    from FDExplicitEu import FDExplicitEu
    options=FDExplicitEu(50,50,0.1,5./12.,0.4,100,100,1000,False)
    print(options.price())

    options=FDExplicitEu(50,50,0.1,5./12.,0.4,100,100,100,False)
    print(options.price())
